...
**RWO golden disk:** the usual golden root PVC is **ReadWriteOnce**. Only **one** virt-launcher at a time can attach that DataVolume. If a second `VirtualMachine` in the same namespace references the same **`ocpv_win_root_dv_name`** while another is still running, paused, or stuck in a restart loop, the new VMI can hit **CrashLoopBackOff** / **Start Failure** / **context deadline exceeded** when scheduling the volume. Before provisioning another VM from the same golden DV, **stop or delete** the other VM (`virtctl stop`, `oc delete vm`, or `spec.running: false`), **or** give each VM its own **cloned** DataVolume (or RWX storage, if your platform supports it for Windows). The create playbook fails fast with a clear message when it detects another VM already referencing the same root DV (override with **`ocpv_skip_rwo_dv_conflict_check: true`** only if you know the storage is safe to share).
