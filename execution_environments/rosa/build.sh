...
_tag=$(date +%Y%m%d)
podman manifest rm quay.io/matferna/madhatter:${_tag}

# create manifest for EE image
podman manifest create quay.io/matferna/madhatter:${_tag}
...
do
    ...
    podman build --platform linux/${arch} \
      ...
      --manifest quay.io/matferna/madhatter:${_tag} . \
      | tee podman-build-${arch}.log
    popd > /dev/null
done

echo "Built quay.io/matferna/madhatter:${_tag}"

# inspect manifest content
#podman manifest inspect quay.io/matferna/madhatter:${_tag}

# tag manifest as latest
#podman tag quay.io/matferna/madhatter:${_tag} quay.io/matferna/madhatter:latest
...
#podman manifest push --all quay.io/matferna/madhatter:${_tag}
#podman manifest push --all quay.io/matferna/madhatter:latest
